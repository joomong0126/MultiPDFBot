...
st.set_page_config(page_title="챗봇", page_icon=":books:", layout="wide")

st.title("📚 챗봇")
st.caption("PDF 관련 질문에 답변합니다")
...
def get_rag_response_streaming(question, vectorstore, api_key, container):
    ...
    try:
        ...
        # 프롬프트 템플릿 설정
        prompt_template = """
        당신은 전문가입니다. 제공된 문서를 바탕으로 정확하고 유용한 답변을 제공해주세요.
...
# 사용자 입력 처리
if user_question := st.chat_input(placeholder="관련 질문을 해주세요... 💬"):
    # API 키 확인
    if not openai_api_key:
